...
int main() {
	...
	VideoCapture capWebcam2(1);

	if (capWebcam.isOpened() == false){cout << "error: Webcam connect unsuccessful\n";	return(0);    }

	if (capWebcam2.isOpened() == false){cout << "error: Webcam connect unsuccessful\n";	return(0);    }
	
	Mat imgOriginal, imgOriginal2;		// Input image
	...
	{					
		...
		bool blnFrameReadSuccessfully2 = capWebcam2.read(imgOriginal);
		...
		{						// for each circle
			...
			capWebcam2.read(frame_); // cam 2
			if (frame_.empty()){cout << "No frame to read" << endl; break;}			
